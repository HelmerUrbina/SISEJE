...
import DataService.Despachadores.Impl.PrioridadDAOImpl;
...
@WebServlet(name = "PrioridadServlet", urlPatterns = {"/Prioridad"})
public class PrioridadServlet extends HttpServlet {
    ...
    private PrioridadDAO objDsArea;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        objDsArea = new PrioridadDAOImpl(objConnection);
        // DE ACUERO AL MODO, OBTENEMOS LOS DATOS NECESARIOS.  
        if (objBnPrioridad.getMode().equals("prioridad")) {
            if (request.getAttribute("objPrioridad") != null) {
                request.removeAttribute("objPrioridad");
            }
            request.setAttribute("objPrioridad", objDsArea.getListaPrioridad());
        }
        if (objBnPrioridad.getMode().equals("U")) {
            objBnPrioridad = objDsArea.getPrioridad(objBnPrioridad);
            result = objBnPrioridad.getDescripcion() + "+++"
                    + objBnPrioridad.getAbreviatura() + "+++"
                    + objBnPrioridad.getEstado();
        }
        //SE ENVIA DE ACUERDO AL MODO SELECCIONADO
        switch (request.getParameter("mode")) {
            case "prioridad":
                dispatcher = request.getRequestDispatcher("Configuracion/Prioridad.jsp");
                break;
            default:
