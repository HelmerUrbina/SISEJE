...
@WebServlet(name = "VerificaSentenciasServlet", urlPatterns = {"/VerificaSentencias"})
public class VerificaSentenciasServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (objBnSentencias.getMode().equals("verificaSentencias")) {
        ...
        switch (request.getParameter("mode")) {
            case "verificaSentencias":
                dispatcher = request.getRequestDispatcher("Resoluciones/VerificaSentencias.jsp");
                break;
            default:
    ...
    private StringBuilder TablaResoluciones(List lista) {
        ...
        if (lista != null) {
            ...
            sb.append("<thead>");
            sb.append("<tr>");
            sb.append("<th style=\"text-align: center\">RESOLUCIONES</th>");
            ...
            for (int k = 0; k < lista.size(); k++) {
                ...
                sb.append("<button type=\"button\" class=\"btn bg-amber btn-xs waves-effect\" data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"Beneficiario\" onclick=\"javascript: RegistrarBeneficiario('").append(resolucion.getResolucion()).append("');\">");
                sb.append("<i class=\"material-icons\">note_add</i>").append("</button> ");
                sb.append("<button type=\"button\" class=\"btn bg-deep-orange btn-xs waves-effect\" data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"Conceptos\" onclick=\"javascript: DetalleResolucion('").append(resolucion.getResolucion()).append("','").append(resolucion.getTipoPago()).append("');\">");
                ...
                sb.append("<button type=\"button\" class=\"btn bg-green btn-xs waves-effect\" data-toggle=\"tooltip\" data-placement=\"bottom\" title=\"Cerrar Resolución\" onclick=\"javascript: CerrarResolucion('").append(resolucion.getResolucion()).append("');\">");
    ...
    private StringBuilder TablaResolucionesDetalle(List lista) {
        ...
        if (lista != null) {
            ...
            for (int k = 0; k < lista.size(); k++) {
                ...
                sb.append("<tr>");
                sb.append("<td><input type=\"checkbox\" id=\"div_checkbox_").append(detalle.getTipoRemuneracion()).append("\" onclick=\"javascript: fn_ActivaDetalle('").append(detalle.getTipoRemuneracion()).append("');\" class=\"chk-col-blue\" disabled /><label for=\"div_checkbox_").append(detalle.getTipoRemuneracion()).append("\">").append(detalle.getOficio()).append("</label></td>");
